...
## Reqiurement files under data folder: 1) rating_small.csv or rating.csv,
##                                      2) combined_info.csv, which has the movie profile: ID, Title, etc.
...
movies = list(rating['movieId'].unique())
...
def get_predictions(user_id):
    ...
    predictions = {}
    rating2 = rating.set_index('userId')
    for i in movies:
        try:
            rating.loc[user_id, i]
        except:
